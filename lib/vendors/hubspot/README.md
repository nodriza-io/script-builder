...
HubSpot uses **Private App** access tokens for authentication. You need to:
...
To associate records (e.g., Contact with Company), use HubSpot's association API separately or include association fields:
...
const contact = await hubspotApi.create('contacts', {
  ...
  associatedcompanyid: '67890'  // Link to company
});
```
