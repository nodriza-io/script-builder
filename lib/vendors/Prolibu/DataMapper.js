...
class DataMapper {
  ...
  static map({ data, map, reverse = false, additionalTransforms = {} } = {}) {
    return this._processMapping({ data, map, reverse, additionalTransforms });
  }
   ...
   * @private
   */
  static _processMapping({ data, map, reverse = false, additionalTransforms = {} }) {
    ...
    if (hasAsyncTransforms) {
      // Handle async case
      return this._handleAsyncTransforms(transformResults, mappingOperations, result);
    } else {
      // Handle sync case
