...
function handleSalesforceAxiosError(err, context = 'Salesforce operation') {
  ...
  // Crear un error estándar de JavaScript que sea serializable
  const salesforceError = new Error(`${context} failed: ${errorMessage}`);
  salesforceError.name = 'SalesforceApiError';
  salesforceError.type = errorType;
  salesforceError.status = statusCode;
  salesforceError.context = context;
  salesforceError.details = errorDetails;
  salesforceError.shouldInvalidateToken = shouldInvalidateToken;
  salesforceError.originalError = err;
  
  return salesforceError;
}

class SalesforceRestApi {
  constructor({ instanceUrl, customerKey, customerSecret, apiVersion = '58.0', sandbox = false, env = 'default' } = {}) {
  ...
  */
  ...
  async find(sobjectType, options = {}) {
    return await this.executeWithRetry(async () => {
      ...
      // validate that all options are strings or numbers
  ...
  Example usage:
    ...
    Returns null if record not found (404) instead of throwing error.
  */
